...
A common use case of shiny apps is to explore a dataset interactively. So we will build a simple app that can create a scatterplot from a dataframe which a user can modify interactively. We will use the built-in `iris` dataset and the user should be able to select x and y axes variables as well the variable mapped to the colour of the points.
...
We start with the standard shiny app template.
...
And to the sidebar, we will add 3 input widgets corresponding to x axis variable, y axis variable and colour variable. These will be dropdown type (`selectInput()`) and choices will be column names of the iris dataframe. And we add the output widget (`plotOutput()`) in the main panel.
...
This was a quick and easy start to shiny apps. You can expand this app to other datasets and map more variables to ggplot etc. Can you add the functionality to [download the plots](https://shiny.rstudio.com/gallery/file-download.html)? Or perhaps to [upload a csv file](https://shiny.rstudio.com/articles/upload.html) and plot from that file?
...
<i class='fa fa-clipboard-list'></i> Try to see if you can get this to work.

```{r,accordion=TRUE,fig.height=6,fig.width=8}
library(shiny)
library(RNASeqPower)
...
Verify that the inputs and outputs work. One can stop at this point if you only need to compute number of samples. But, we will continue to enchance the app to be able to compute any of the other parameters.
...
<i class='fa fa-clipboard-list'></i> Try to add a selection based conditional UI. `?selectInput()`, `?uiOutput()`, `?renderUI()`.
...
Let's summarise what we have done. In the **ui** part, instead of directly including the `rnapower()` input variable in the `sidebarPanel()`, we have added a selection input and a conditional UI output. The content inside the output UI depends on the selection made by the user.
...
<i class='fa fa-clipboard-list'></i> Update the app such that all numeric inputs are replaced by text inputs and the server logic is able to parse the strings into numbers.
...
<i class='fa fa-clipboard-list'></i> Try to figure out how the code can be reorganised and simplified.
...
<i class='fa fa-clipboard-list'></i> Try entering some unreasonable inputs and see what happens. Try entering random strings, or huge integers for **alpha** etc.
...
<i class='fa fa-clipboard-list'></i> Try to figure out how to add input validation. `?shiny::validate()`.
...
<i class='fa fa-clipboard-list'></i> Test that the validation works by inputting unreasonable inputs such as alphabets, symbols etc into the input fields.
...
<i class='fa fa-clipboard-list'></i> Try changing the theme and pick one that you like. Check out `?shinythemes`. The themes can be visualised on [Bootswatch](https://bootswatch.com/).
...
This should now look like this. Some additional syling has been added to divs.

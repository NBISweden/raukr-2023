...
:::: {.columns}
::: {.column width="50%"}
![](assets/hex-tidyverse.png){.center width="30%"}
...
We can explicitly tell what parser to use:
...
But knowing that the **parser is guessed based on the first 1000 lines**, we can see what sits past the 1000th line in the data:

```{r}
tricky_dataset %>% head(n = 1002) %>% tail(n = 4)
```

It seems, we were very unlucky, because up till the 1000th line there are only integers in the `x` column and `NA`s in the `y` column so the parser cannot be guessed correctly. To fix this:
